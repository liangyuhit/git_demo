...
window_gaussian = signal.gaussian(51, std=1)
...
window_gaussian_2 = signal.gaussian(51, std=7)
A_gaussian_2 = fft(window_gaussian_2, 2048) / (len(window_gaussian_2)/2.0)
freq_gaussian_2 = np.linspace(-0.5, 0.5, len(A_gaussian_2))
response_gaussian_2 = 20 * np.log10(np.abs(fftshift(A_gaussian_2 / abs(A_gaussian_2).max())))
...
plt.title("Gaussian window ($\sigma$=1)")
plt.ylabel("Amplitude")
plt.xlabel("Sample")
plt.grid(which='major', axis='both')
plt.subplot(6,2,10)
...
plt.axis([-0.5, 0.5, -120, 0])
plt.title(r"Frequency response of the Gaussian window ($\sigma$=1)")
plt.ylabel("Normalized magnitude [dB]")
plt.xlabel("Normalized frequency [cycles per sample]")
plt.grid(which='major', axis='both')

plt.subplot(6,2,11)
plt.plot(window_gaussian_2)
plt.title("Gaussian window ($\sigma$=7)")
plt.ylabel("Amplitude")
plt.xlabel("Sample")
plt.grid(which='major', axis='both')
plt.subplot(6,2,12)
plt.plot(freq_gaussian_2, response_gaussian_2)
plt.axis([-0.5, 0.5, -120, 0])
